...
- [ ] 建立统一命令系统和快捷键配置，并接入菜单、上下文菜单、标题栏按钮和命令面板；当前已完成基础命令注册表、可替换快捷键映射、事件匹配、提示生成、命令状态快照和面板内命令面板，仍缺用户配置持久化和全局菜单状态同步。
- [ ] 实现递归搜索和搜索结果面板，已支持名称/扩展名、大小、修改时间、文件/目录类型、取消、扫描进度和结果打开；仍需补内容搜索、索引策略和协议能力过滤。
