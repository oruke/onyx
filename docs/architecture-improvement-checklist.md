...
- [ ] 错误可见性仍需加强：文件监听降级、批量重命名正则错误、远程连接测试、面板加载、搜索、常见文件任务、平台打开方式失败和文本预览失败已接入可见或结构化错误消息，可区分认证、权限、网络、未找到、已存在和协议不支持；缩略图异常已进入日志，画廊与检查器都有基础占位，仍需继续收敛图片元数据和更多平台服务失败的 UI 展示。
...
- [x] 补 WebDAV 基础写命令：WebDAV provider 已实现 `PUT` 空文件创建、`MKCOL` 创建目录、`DELETE` 删除、`MOVE` 重命名、`COPY`/`MOVE` 同端点复制移动、`GET` 内容读取和 `PUT` 内容写入，并通过可路由命令服务与内容服务接入。
...
- [ ] 实现递归搜索和搜索结果面板，已支持名称/扩展名、大小、修改时间、文件/目录类型、本地内容过滤、取消、扫描进度、结果打开和内容搜索能力预检；仍需补索引策略和远程内容搜索。
- [ ] 完善任务队列：已补失败明细、失败任务重试、速度估算、剩余时间、任务历史跨启动恢复和完成任务历史归档；仍缺真正可续跑的持久化执行队列、限速和并发限制。
- [ ] 增加 Undo/Redo 操作日志，至少覆盖移动、重命名、批量重命名和删除到回收站。
- [ ] 增强预览和检查器：VFile 权限/能力展示、压缩包加密/只读能力提示、本地文件和压缩包条目的 SHA-256 摘要已接入；仍需补 PDF、音视频、编码选择、EXIF 和更完整的大文件取消策略。
