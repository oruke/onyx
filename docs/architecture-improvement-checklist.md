...
- [ ] 命令体系不完整：基础文件操作已有轻量 `OnyxCommand` 快捷键描述和事件匹配入口，快捷键提示可由命令元数据生成；但仍缺完整 Command Registry、快捷键配置、命令面板和菜单状态同步。
- [ ] 跨 provider 文件命令不足：本地与 SMB 已能通过 `ProviderBackedFileCommandService` 分发同 provider 命令，WebDAV/S3 写入能力差异已有明确错误；但远程协议之间、压缩包内部写入、远程到本地、本地到远程等场景仍无法统一表达。
- [ ] 任务系统缺少持久化队列：任务中心可显示、暂停、取消和自动清理，但没有重试、恢复、历史记录、失败明细聚合和跨启动恢复。
- [ ] 错误可见性仍需加强：文件监听降级和批量重命名正则错误已可见；但部分异常仍转成原始字符串或静默忽略，用户难以区分权限、认证、网络、文件冲突、路径非法和协议不支持。
- [ ] 平台能力不均衡：终端打开已按 Windows、macOS、Linux 生成平台候选命令并保留 `TERMINAL` 环境变量优先；但 `OpenWithService` 当前仍是 Linux `.desktop` 实现，回收站依赖 Desktop API，跨平台行为仍需要真实系统验证。
...
- [ ] 建立统一命令系统和快捷键配置，并接入菜单、上下文菜单、标题栏按钮和命令面板；当前只完成基础快捷键描述、事件匹配和提示生成入口。
